...
export function TeamSection({ members }: TeamSectionProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const transitionTo = (next: number, prev: number) => {
        ...
        // ── Right rail: all rows update simultaneously ─────────────────
        railRows.forEach((row, i) => {
          const name = row.querySelector("[data-rail-name]");
          const role = row.querySelector("[data-rail-role]");
  ...
  return (
    ...
    >
      {/* ── Ambient background: giant faint TEAM lettering ─────────── */}
      <div
        aria-hidden="true"
      ...
      {/* ── Main body: left panel + right rail ─────────────────────── */}
      <div className="absolute inset-0 top-17 bottom-11 flex">
        {/* ══════════════════════════════════════
            LEFT PANEL — active member showcase
        ══════════════════════════════════════ */}
        ...
        {/* ══════════════════════════════════════
            RIGHT RAIL — all members (desktop only)
        ══════════════════════════════════════ */}
